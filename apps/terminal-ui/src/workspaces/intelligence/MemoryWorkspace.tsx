...
export function MemoryWorkspace({events,onEvent,selectedThesis,onThesis}: {events:ReleaseSummary[];onEvent:(id:string)=>void;selectedThesis:string|null;onThesis:(id:string)=>void}) {
  ...
  return <div class="intelligence-workspace"><header class="section-heading"><div><span class="section-label">RESEARCH MEMORY</span><h1>让每次研究留下来</h1></div></header><div class="tabs-bar"><button type="button" class={tab==='events'?'active':''} onClick={()=>setTab('events')}>事件档案</button><button type="button" class={tab==='notes'?'active':''} onClick={()=>setTab('notes')}>宏观推理</button></div>
