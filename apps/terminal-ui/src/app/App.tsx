...
function CountryDrawerContent({ detail, advanced, learningMode, onOpenMarket, onOpenEvent, onOpenSeries }: { detail: ProductCountryDetail; advanced: boolean; learningMode: boolean; onOpenMarket: (item: ProductMarketItem) => void; onOpenEvent: (id: string) => void; onOpenSeries: (key: string) => void }) {
  ...
  return <>
    ...
    <Panel title={dimension ? "主要驱动" : "关键序列"} eyebrow="OBSERVED INPUTS"><div class="research-list research-list--compact">{series.map((item) => <button type="button" class="research-row" key={item.key} onClick={() => onOpenSeries(item.key)} aria-label={`查看序列：${item.title}`}><div><strong>{item.title}</strong><span>{item.dimension_label} · {item.period_start ?? "时间未记录"}</span></div><span>{item.latest_value == null ? "—" : item.latest_value.toLocaleString()} →</span></button>)}{!series.length ? <p class="muted">暂无可用关键序列。</p> : null}</div></Panel>
...
export function App() {
  ...
  return <div class={collapsed?"terminal-shell intelligence-shell terminal-shell--collapsed":"terminal-shell intelligence-shell"}>
    ...
    {view==="macro"&&selectedCountryKey?<Drawer title="宏观环境" onClose={closeDrawer}><ReadStatus label="国家" resource={country} advanced={advancedMode}/>{country.data?<CountryDrawerContent detail={country.data} advanced={advancedMode} learningMode={learningMode} onOpenMarket={openMarket} onOpenEvent={openEvent} onOpenSeries={openSeries}/>:null}</Drawer>:null}
