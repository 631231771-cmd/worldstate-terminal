...
function MarketDrawerContent({ item, advanced, learningMode }: { item: ProductMarketItem; advanced: boolean; learningMode: boolean }) {
  ...
  return <>
    ...
    <Panel title="研究关联" eyebrow="MACRO CONTEXT"><div class="context-links"><span>实际利率</span><span>美元</span><span>通胀</span><span>流动性</span></div><ConceptStrip concepts={["real_yield", "inflation", "liquidity"]} active={learningMode} /></Panel>
...
export function App() {
  ...
  const drawer = selectedMarket ? { title: selectedMarket.label, body: <MarketDrawerContent item={selectedMarket} advanced={advancedMode} learningMode={learningMode} /> } : countryDetail ? { title: countryDetail.selected_dimension ? `${countryDetail.country.label} · ${countryDetail.selected_dimension.label}` : countryDetail.country.label, body: <CountryDrawerContent detail={countryDetail} advanced={advancedMode} learningMode={learningMode} onOpenMarket={openMarket} onOpenEvent={(id) => void openEvent(id)} /> } : seriesDetail ? { title: String(seriesDetail.title ?? selectedSeriesKey ?? "Series"), body: <SeriesDrawerContent data={seriesDetail} /> } : null;
