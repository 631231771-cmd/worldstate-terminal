...
const markets={...base,items:[market('gold_gc','黄金'),market('wti_cl','WTI 原油'),market('ust2y_yield_context','美国 2Y','bp'),market('dollar_broad_context','美元')],limitations:[]};
...
(async()=>{
 ...
 try {
  ...
  await context.route('**/v2/**',async route=>{
   ...
   if(p==='/v2/product/quotes')return route.fulfill({json:{as_of:new Date().toISOString(),refresh_seconds:60,items:[{key:'xau_usd',label:'黄金现货参考',symbol:'XAU',kind:'spot_indicative',unit:'USD/盎司',price:4000,change:null,change_unit:'%',quoted_at:new Date().toISOString(),retrieved_at:new Date().toISOString(),delay_minutes:null,status:'indicative',error:null,source_url:'https://gold-api.com/',limitation:'仅用于隔离浏览器测试',points:[{time:new Date(Date.now()-60000).toISOString(),value:3999},{time:new Date().toISOString(),value:4000}]}]}});
   if(p==='/v2/product/headlines')return route.fulfill({json:{as_of:new Date().toISOString(),source_scope:'仅官方消息',items:[{title:'Official energy update',url:'https://www.eia.gov/',source:'EIA',topic:'energy',published_at:new Date().toISOString(),retrieved_at:new Date().toISOString()},{title:'Official policy update',url:'https://www.federalreserve.gov/',source:'Federal Reserve',topic:'policy',published_at:new Date().toISOString(),retrieved_at:new Date().toISOString()}]}});
   if(p.startsWith('/v2/product/factors/'))return route.fulfill({json:{asset:p.split('/').at(-1),as_of:new Date().toISOString(),items:[{label:p.endsWith('/oil')?'美国商业原油库存':'美国 10 年实际收益率',series_key:p.endsWith('/oil')?'eia.crude_stocks':'treasury.real_10y',value:420,previous_value:421,unit:'million_barrels',frequency:'weekly',period:'2026-08-07',available_at:'2026-08-11T12:00:00Z',source_url:'https://www.eia.gov/',observed:true,proxy:false,point_in_time:false,limitation:'current version'}]}});
   if(p==='/v2/reasoning/evidence/sync'&&route.request().method()==='POST')return route.fulfill({json:{items:[]}});
  ...
  await page.goto(origin+'/#radar');await visible('.desk-asset');await page.getByText('Official policy update').first().waitFor();
  await page.locator('.desk-asset').filter({hasText:'黄金'}).first().click();await visible('.desk-focus .timeseries-chart');assert.equal((await page.locator('.desk-focus__number').innerText()).includes('-0.50'),false,'spot quote cannot borrow futures daily change');
  await page.locator('.desk-asset').filter({hasText:'WTI 原油'}).click();await page.getByText('美国商业原油库存').waitFor();await page.getByText('Official energy update').last().waitFor();console.log('PASS asset-linked market, official info and factors');
  assert.equal(await page.locator('nav[aria-label="主导航"] button').count(),4);
  await page.getByRole('button',{name:'学习',exact:true}).click();console.log('PASS workbench, source age and learning toggle');
  ...
  await page.goto(origin+'/#radar');await visible('.desk-asset');eventsFail=true;
  await page.getByRole('button',{name:'刷新',exact:true}).click();await page.getByText(/事件刷新失败/).waitFor();assert.ok(await page.locator('.desk-asset').count()>=3);
  console.log('PASS partial API failure preserves market workspace');
  eventsFail=false;offline=true;await page.reload();await visible('.desk-asset');await page.getByText(/市场刷新失败/).waitFor();assert.equal(await page.locator('nav button').count(),4);console.log('PASS offline reopen uses saved observed state');
  offline=false;fixture=true;await page.getByRole('button',{name:'刷新',exact:true}).click();await page.getByText(/市场刷新失败/).waitFor();assert.ok(await page.locator('.desk-asset').count()>=3);fixture=false;
  ...
  for(const size of [{width:1440,height:900},{width:1920,height:1080}]){await page.setViewportSize(size);await page.goto(origin+'/#radar');await visible('.desk-asset');assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:path.join(root,`test-results/market-desk-${size.width}.png`)});}
