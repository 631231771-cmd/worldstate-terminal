...
WINDOW_SPECS = (
    ...
    WindowSpec("us_cash_close", "T0至美国现金市场收盘", 0, 27000, True),
    WindowSpec("next_close", "下一交易日收盘", 0, 113400, True),
    WindowSpec("day_5_close", "5个交易日后", 0, 545400, True),
)
...
def _calculate_one(
    ...
    *,
    ...
    end_at = release_at + timedelta(seconds=spec.end_seconds)
    ...
    duration = max(interval_seconds, spec.end_seconds - spec.start_seconds)
